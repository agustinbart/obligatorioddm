...
const DropDownTreatmentsNA = (props) => {
    ...
    useEffect(() => {
        db.transaction((tx) => {
          tx.executeSql(`SELECT t.tratamiento_id FROM treatments t WHERE t.tratamiento_id NOT IN (SELECT r.tratamiento_id FROM replacements r) AND t.tratamiento_id NOT IN(SELECT s.tratamiento_id FROM supplies s)`, [], (tx, results) => {
